...
- preprocessing_report.json: Summary statistics and random seed info
...
class Config:
    ...
    PREPROCESSING_REPORT_JSON = DATA_DIR / "out/preprocessing_report.json"
...
def save_preprocessed_data(
    ...
    """
    Save preprocessed data and generate report.
    ...
    report = {
        ...
        "n_districts_sampled": len(sampled_gdf),
        "target_year": Config.TARGET_YEAR,
        "aggregate_subgroup": Config.AGGREGATE_SUBGROUP,
        "outcome_columns": Config.OUTCOME_COLS,
        "data_sources": {
            "seda_csv": str(Config.SEDA_CSV),
            "shapefile": str(Config.SHAPEFILE)
        },
        "output_files": {
            "sampled_districts_csv": str(Config.SAMPLED_DISTRICTS_CSV),
            "preprocessed_geojson": str(Config.PREPROCESSED_GEOJSON),
            "report_json": str(Config.PREPROCESSING_REPORT_JSON)
        }
    }
    
    # Add outcome statistics
    report["outcome_statistics"] = {}
    for col in Config.OUTCOME_COLS:
        if col in sampled_gdf.columns:
            report["outcome_statistics"][col] = {
                "mean": float(sampled_gdf[col].mean()),
                "std": float(sampled_gdf[col].std()),
                "min": float(sampled_gdf[col].min()),
                "max": float(sampled_gdf[col].max()),
                "count": int(sampled_gdf[col].notna().sum())
            }
    
    # Save report
    with open(Config.PREPROCESSING_REPORT_JSON, "w") as f:
        json.dump(report, f, indent=2)
    logger.info(f"Saved preprocessing report to {Config.PREPROCESSING_REPORT_JSON}")
...
def run_preprocessing():
    ...
    try:
        ...
        report = save_preprocessed_data(sampled_gdf, Config.RANDOM_SEED)
        ...
        logger.info(f"  - {Config.PREPROCESSING_REPORT_JSON}")
        
        logger.info("\n" + "="*80)
        logger.info("DATA PREPROCESSING COMPLETED SUCCESSFULLY")
        logger.info("="*80)
        
        return sampled_gdf, report
...
if __name__ == "__main__":
    ...
    # Run preprocessing
    sampled_gdf, report = run_preprocessing()
