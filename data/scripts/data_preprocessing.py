...
class Config:
    ...
    SEDA_CSV = RAW_DATA_DIR / "seda_geodist_annual_cs_6.0.csv"
    SHAPEFILE = RAW_DATA_DIR / "EDGE_SCHOOLDISTRICT_TL25_SY2425.shp"

    # Output paths
    SAMPLED_DISTRICTS_CSV = PROCESSED_DATA_DIR / "sampled_districts.csv"
    PREPROCESSED_GEOJSON = PROCESSED_DATA_DIR / "preprocessed_data.geojson"
...
def trim_merged_columns(merged_gdf: gpd.GeoDataFrame) -> gpd.GeoDataFrame:
    """Drop unused variables before sampling and saving."""
    keep_cols = [
    ...
    keep_cols.extend(Config.OUTCOME_COLS)
    keep_cols.extend(Config.OUTCOME_SE_COLS)
    keep_cols.extend(Config.COLS_TO_KEEP)

    keep_cols = [col for col in keep_cols if col in merged_gdf.columns]
    trimmed_gdf = merged_gdf[keep_cols].copy()

    dropped_cols = [col for col in merged_gdf.columns if col not in keep_cols]
    logger.info(
        f"Trimmed merged dataset to {len(keep_cols)} columns; "
        f"dropped {len(dropped_cols)} unused columns"
    )
    if dropped_cols:
...
def run_preprocessing():
    ...
    try:
        ...
        merged_gdf = trim_merged_columns(merged_gdf)
